...
```
**The higher number of observations makes the plot more normally distributed and it has a lower variance (makes it narrower). The ones with less samples is slightly skewed right.**
...
**The 'delta = 0' specifies no difference in the null model.**
...
**It is a normal distribution centered at zero because the null hypothesis says there is no (zero) difference in average ratings between conditions.**
...
**It is the simulation number (1-1000).**
...
**It's centered at zero because the null says no difference.**
...
**The confidence interval does NOT contain zero (null value).**
...
as well as SEs, and visualize how they're different
...
Between subjects experiment -- lots of variability! I like to visualize the raw data as well as the mean and CIs
...
```{r}
ggplot(data = tea_data_highn, 
       aes(x=condition, y=rating, col=condition))  +
  geom_jitter(width=.1, height=0, alpha=.5) + # visualizes all the raw data, with no variation in y-axis jitter
  theme_few() +
  geom_pointrange(data = tea_data_highn_summary_cis, 
                  aes(x=condition, y = cond_mean, 
...
**Each dot represents an individual simulation mean difference value; it is the raw data. The range represents the tea rating values.**


To do: how does this change when you use the low-n experiment?

**There are less points on the graph, and the standard error is often larger which also leads to wider confidence intervals.**
...
**The proportion of simulations that have p-values less than 0.05 is 0.07. Our false positive rate is 0.07.**
...
**This distribution is very skewed left.**
